  ...
  return (
    <div className='p-6 max-w-6xl mx-auto bg-white shadow-lg rounded-lg'>
      ...
      {!loading && ticket.length === 0 && (
        <p className='text-red-500 mb-4'>Không có vé nào được tìm thấy.</p>
      )}
         ...
         <div className='space-y-6 border-b-3 pb-6 mb-6' key={t.id}>
          <div className='bg-gray-100 p-4 rounded-lg'>
            ...
            <div className='grid grid-cols-2 md:grid-cols-3 gap-4 text-sm text-gray-700'>
              <div><strong>Mã vé:</strong> {t.id}</div>
              <div><strong>Điểm đi:</strong> {t.diemdi}</div>
              <div><strong>Điểm đến:</strong> {t.diemden}</div>
              <div><strong>Layout ghế:</strong> {t.seatLayout}</div>
              <div><strong>Loại xe:</strong> {t.type}</div>
              <div><strong>Bắt đầu:</strong> {t.starttime}</div>
              <div><strong>Kết thúc:</strong> {t.endtime}</div>
              <div><strong>Thời gian đi:</strong> {t.timetogo}</div>
              <div><strong>Ngày nghỉ:</strong> {t.offday}</div>
          ...
          <div className='bg-white p-4 rounded-lg shadow'>
            ...
            <div className='grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-4 text-sm'>
              {t.seat?.map((s: any) => (
                <div
                  key={s.id}
                  className='bg-blue-50 p-2 border border-blue-200 rounded text-center shadow-sm'
                >
